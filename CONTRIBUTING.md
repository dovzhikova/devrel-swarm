...
```bash
git clone https://github.com/dovzhikova/devrel-origin.git
cd devrel-origin
...
```
src/devrel_origin/core/   # 12 agent implementations + shared base
...
src/devrel_origin/tools/  # External integrations (GitHub, Search, Email, etc.)
...
### Add a New Agent
1. Create `src/devrel_origin/core/new_agent.py` with an `execute(task, context)` async method
...
### Add an Integration
1. Create `src/devrel_origin/tools/new_tool.py` with an async client class
...
4. Run `python -m devrel_origin.core.atlas --weekly-cycle`
...
ruff check .         # Lint
black .              # Format
mypy src/devrel_origin/  # Type check
```
