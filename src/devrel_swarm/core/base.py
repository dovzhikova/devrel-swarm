...
def load_agent_prompt(agent_name: str, prompt_name: str, default: str) -> str:
    """Load a prompt from optimize/{agent_name}/{prompt_name} if it exists.

    Falls back to the inline default. Also appends known_issues.txt if
    present (auto-generated by the self-improvement loop).
    """
    # Load base prompt
    path = _OPTIMIZE_DIR / agent_name / prompt_name
    if path.exists():
        logger.info(f"Loaded optimized prompt: {path}")
        prompt = path.read_text(encoding="utf-8")
    else:
        flat_path = _OPTIMIZE_DIR / prompt_name
        prompt = flat_path.read_text(encoding="utf-8") if flat_path.exists() else default

    # Append known issues if self-improvement has generated them
    issues_path = _OPTIMIZE_DIR / agent_name / "known_issues.txt"
